...
import { checkCollision, checkWallCollision, checkFoodCollisionRecursive, } from "./utils/collision.js";
export class Game {
    constructor(canvas, context) {
        ...
        this.initialUpdateInterval = 150;
        ...
        this.eatSound.volume = 0.3; // Define o volume como 50%
        this.collisionSound.volume = 0.1; // Define o volume como 50%
    ...
    showCountdown() {
        ...
        this.context.fillText(this.countdownValue.toString(), this.canvas.width / 2, this.canvas.height / 2);
    ...
    isFoodOnSnakeBody(index) {
        ...
        if (index >= body.length)
            return false;
        if (body[index].x === this.food.position.x &&
            body[index].y === this.food.position.y) {
            return true;
        }
        return this.isFoodOnSnakeBody(index + 1);
    }
    checkScoreMilestone(currentScore, milestone) {
    ...
    drawSnake() {
        ...
        body.forEach((segment, index) => {
            ...
            if (index === 0) {
                ...
                this.context.beginPath();
                this.context.arc(x + eyeOffsetX, y + eyeOffsetY, eyeRadius, 0, Math.PI * 2);
                this.context.arc(x - eyeOffsetX, y + eyeOffsetY, eyeRadius, 0, Math.PI * 2);
                this.context.fill();
                this.context.fillStyle = "#000000";
                this.context.beginPath();
                this.context.arc(x + eyeOffsetX, y + eyeOffsetY, eyeRadius / 2, 0, Math.PI * 2);
                this.context.arc(x - eyeOffsetX, y + eyeOffsetY, eyeRadius / 2, 0, Math.PI * 2);
                this.context.fill();
            }
    ...
    drawFood() {
        if (this.food.position.x >= 0 && this.food.position.y >= 0) {
            ...
            const gradient = this.context.createRadialGradient(x - 3, y - 3, 0, x, y, this.cellSize / 2);
            gradient.addColorStop(0, "#FFFFFF");
            gradient.addColorStop(0.5, "#FFD700");
            gradient.addColorStop(1, "#DAA520");
            this.context.fillStyle = gradient;
            this.context.beginPath();
            this.context.ellipse(x, y, this.cellSize / 2 - 4, this.cellSize / 3 - 2, 0, 0, Math.PI * 2);
            this.context.fill();
            this.context.shadowColor = "rgba(0, 0, 0, 0.5)";
            this.context.shadowBlur = 5;
            this.context.fill();
            ...
            this.context.beginPath();
            this.context.ellipse(x - 5, y - 5, 3, 2, Math.PI / 4, 0, Math.PI * 2);
            this.context.fill();
        }
    }
    updateScoreDisplay() {
    ...
    endGame() {
        ...
        this.context.fillText(`Pontuação: ${finalScore}`, this.canvas.width / 2, this.canvas.height / 2 + 20);
        this.context.font = "18px 'Press Start 2P', cursive";
        this.context.fillText("Escolha o modo para reiniciar", this.canvas.width / 2, this.canvas.height / 2 + 60);
        this.context.shadowBlur = 0;
